...
import Foundation
import SpriteKit
public struct Reversi: Codable, CustomStringConvertible{
    ...
    public enum ability: Int, CaseIterable, Codable{
        case none = 0, translate, clearBomb, colorBomb
    }
    public var didLastTurnUseAbility = false
    private var abilityCoolDown = ["white": 0, "black": 0]
    public mutating func setAbilityCoolDown(isWhite: Bool, duration: Int, pr: Bool = false){
        if isWhite{
        ...
        if pr{print("now cool down white: ", abilityCoolDown["white"]!, "black: ", abilityCoolDown["black"]!)}
    }
    public func getAbilityCoolDown(isWhite: Bool) -> Int{
        if isWhite{
            return abilityCoolDown["white"] ?? 0
        }
        return abilityCoolDown["black"] ?? 0
    }
    public static var translateDx = 1
    public static var translateDy = 0
    public static var withAbility = ability.none
    public enum condition: Int, CaseIterable, Codable{
        case none = 0, translate, clearBomb, colorBomb
    }
    ...
    public func boardToString() -> String{
        ...
        for row in 0...n - 1
        {
            ...
            for col in 0...n - 1
            {
                ...
                else if (self.isAvailable(Row: row, Col: col, isWhite: isColorWhiteNow)) {output += " ?|"}
                else {output += "  |"}
            }
            output += "\n"
        }
    ...
    public mutating func play(Row row: Int, Col col: Int, isComputerWhite: Bool? = nil){
    ...
    public func bestSolutionWithoutAlphaBetaCut(isWhite: Bool, searchDepth: UInt = 1, stopFinding: inout Bool, weight: Weight = Weight(), isComputerWhite: Bool? = nil) -> chessBoardPos?{
    ...
    public mutating func bestSolRecur(isWhite: Bool, searchDepth: UInt = 1, stopFinding: inout Bool, weight: Weight = Weight(), isComputerWhite: Bool? = nil) -> chessBoardPos?{
        let currentCondition = self.currentCondition
        let abilityCoolDown = getAbilityCoolDown(isWhite: isColorWhiteNow)
        if abilityCoolDown == 0, Reversi.withAbility == .translate, currentCondition == .translate, isComputerWhite == isColorWhiteNow{
            return nil
        ...
        func alphaBetaMinMaxScore(isMaximizer: Bool, alpha: Int? = nil, beta: Int? = nil, game: Reversi, depth: UInt) -> Int{
            if depth == 0 || game.isEnd(){
                //print(game.evaluation(weight: weight))
                return game.evaluation(weight: weight)
            }
            
            let abilityCoolDown = game.getAbilityCoolDown(isWhite: game.isColorWhiteNow)
            let needToBreak = abilityCoolDown == 0 && Reversi.withAbility == .translate && game.currentCondition == .translate && isComputerWhite == game.isColorWhiteNow
            if isMaximizer{
                var score: Int? = nil
                var newAlpha: Int? = alpha
                for step in game.availableSteps(isWhite: game.isColorWhiteNow){
                    //print(String(repeating: "#", count: Int(depth)*5),depth,"(\(step.row),\(step.col))")
                ...
                for step in game.availableSteps(isWhite: game.isColorWhiteNow){
                    ...
                    if stopFinding {return -1}
                    var gameTest = game
                    gameTest.play(Row: step.row, Col: step.col, isComputerWhite: isComputerWhite)
                    let isMaximizer = (gameTest.isColorWhiteNow == isColorWhiteNow)
        ...
        for step in self.availableSteps(isWhite: isColorWhiteNow){
            //print(String(repeating: "#", count: Int(searchDepth)*5),searchDepth,"(\(step.row),\(step.col))")
            var game = self
            game.play(Row: step.row, Col: step.col, isComputerWhite: isComputerWhite)
        ...
        //posVSscore.map{print($0.key,$0.value)}
        guard let scoreMax = posVSscore.values.max() else {return nil}
        return posVSscore.filter{$0.value == scoreMax}.keys.randomElement()
    }
    public mutating func bestSolution(isWhite: Bool, searchDepth: UInt = 1, stopFinding: inout Bool, weight: Weight = Weight(), isComputerWhite: Bool? = nil) -> chessBoardPos?{
        let currentCondition = self.currentCondition
        let abilityCoolDown = getAbilityCoolDown(isWhite: isColorWhiteNow)
        if abilityCoolDown == 0, Reversi.withAbility == .translate, currentCondition == .translate, isComputerWhite == isColorWhiteNow{
            return nil
        ...
        for i in 0...firstStepGames.count - 1 {
            ...
            while !nodesToVisit.isEmpty{
                ...
                if currentDepth < searchDepth - 1 && !currentNode.isEnd(){
                    ...
                    /////m
                    let abilityCoolDown = currentNode.getAbilityCoolDown(isWhite: currentNode.isColorWhiteNow)
                    let needToBreak = abilityCoolDown == 0 && Reversi.withAbility == .translate && currentNode.currentCondition == .translate && isComputerWhite == currentNode.isColorWhiteNow
                    numberOfScoresToCalculateStack.insert(needToBreak ? 1 : sampleOfAvailableSteps.count, at: 0)
                    for availableStep in sampleOfAvailableSteps{
                        var testGame = currentNode
                        print(String(repeating: "#", count: currentDepth),"depth \(currentDepth):","(\(availableStep.row),\(availableStep.col))")
                        testGame.play(Row: availableStep.row, Col: availableStep.col, isComputerWhite: isComputerWhite)
                    ...
                    for availableStep in samepleOfAvailableSteps{
                        ...
                        print("end : ","(\(availableStep.row),\(availableStep.col))", currentEvaluation)
        ...
        firstStepMinLastScore.map{print($0.key,$0.value)}
        let max = firstStepMinLastScore.values.max()
        return firstStepMinLastScore.filter({$0.value == max}).keys.randomElement()
    }
